...
class ChatBotView(APIView):
    ...
    def process_user_query(self, user_input, session_id):
        ...
        try:
            ...
            msg_history_instance = msg_history.first()
            # Check if an instance was found
            if msg_history_instance:
                # Access the text attribute from the instance
                text_entries = msg_history_instance.text
                msg_history_list = [{'sender': message.get('sender'), 'content': message.get('content')} for message in text_entries]
                ...
                msg_history_instance.text = msg_history_list
                msg_history_instance.save()
            else:
                print("No MessagesHistory instance found for session_id:", session_id)
...
#             # Generate embeddings and create a vector store
#             vectorstore = self.vector_store(text_chunks)
#             print("Text Chunks:", vectorstore)
#
#             # Get or initialize the conversation chain from the session
#             conversation_chain = self.get_conversation_chain(vectorstore)
#             print("Text Chunks:", conversation_chain)
#
#             return Response(file_serializer.data, status=status.HTTP_201_CREATED)
#         else:
#             return Response(file_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#             file_serializer = UploadedFileSerializer(data=self.request.data)
#             assistant_message = {'role': 'assistant', 'content': 'Error in processing user query'}
#             result = {}  # Initialize result with an empty dictionary
#
#             if file_serializer.is_valid():
#                 file_serializer.save(session_id=session_id)
#                 # Fetching parsed content from FileUploadView
#                 content = self.file_upload_view.parse_and_save_content(file_serializer.instance.file.path,
#                                                                               session_id)
#
#                 # Perform operations using the parsed content
#                 # For example, print the parsed content
#                 print("Parsed Content:", content)
#                 # Fetch text chunks from the parsed and saved content
#                 text_chunks = self.file_upload_view.get_text_chunks(content)
#                 vectordb = self.file_upload_view.vector_store(text_chunks)
#                 conversation_chain = self.file_upload_view.get_conversation_chain(vectordb)
#             else:
#                 print("Warning: file_serializer is not valid. Additional details:", file_serializer.errors)
#
...
#                 text_entries = msg_history_instance.text
#
#                 # Your existing code to create msg_history_list
#                 msg_history_list = [
#                     {'role': message.get('role'), 'content': message.get('content')} for message in text_entries
#                 ]
#
#                 msg_history_list.append({'role': 'user', 'content': user_input})
#
#                 # If conversation_chain is not None, update assistant_message
#                 if conversation_chain is not None:
#                     result = conversation_chain({"question": user_input, "chat_history": msg_history_list})
#                     assistant_message = {'role': 'assistant', 'content': result.get("answer", "")}
#                 else:
#                     print("Warning: conversation_chain is None")
#
#                 # Append the assistant's message to the list
#                 msg_history_list.append(assistant_message)
#
#                 # Update the 'text' field of the MessagesHistory object
#                 msg_history_instance.text = msg_history_list
#                 msg_history_instance.save()
#             else:
#                 print("No MessagesHistory instance found for session_id:", session_id)
...
#         except Exception as e:
#             logging.error("Error in process_user_query: %s", e)
#             raise  # Re-raise the exception to propagate it and trigger a 500 response
#
#
#
#
